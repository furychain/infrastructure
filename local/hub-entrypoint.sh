...
init_chain() {
    ...
    cp ~/.furya/config/gentx/* /home/shared/gentx/
    # ---------------------------------------------------------------------------- #
    #                                 update config                                #
    # ----------------------------------------------------------------------------
...
create_genesis() {
    ...
    cp ~/.furya/config/genesis.json /home/shared/gentx/
}
...
wait_for_genesis() { 
    ...
    cp /home/shared/gentx/genesis.json ~/.furya/config/
}
...
add_peers_to_config() {
    ...
    for file in /home/shared/peers/*; do
        ...
        sed -i "s/persistent_peers = \"\"/persistent_peers = \"$(cat $file),\"/g" ~/.furya/config/config.toml
    done

    # Remove the last comma from the persistent peers
    sed -i "s/,\"/\"/g" ~/.furya/config/config.toml
}
